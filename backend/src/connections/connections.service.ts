...
export class ConnectionsService {
    ...
    async inviteConnection(inviterWorkspaceId: string, dto: InviteConnectionDto) {
        ...
        const existing = await this.connectionsRepository.findOne({
            where: [
                { inviterWorkspaceId, inviteEmail: dto.email, status: ConnectionStatus.PENDING },
                { inviterWorkspaceId, inviteEmail: dto.email, status: ConnectionStatus.ACCEPTED },
            ],
        });

        if (existing) {
            throw new BadRequestException('Ya existe una conexión con este correo electrónico.');
        }
    ...
    async testGetPendingInvitesAndConnections(workspaceId: string) {
        ...
        const pendingSent = await this.connectionsRepository.find({
            where: { inviterWorkspaceId: workspaceId, status: ConnectionStatus.PENDING },
    ...
    async acceptConnection(token: string, inviteeWorkspaceId: string, userEmail: string) {
