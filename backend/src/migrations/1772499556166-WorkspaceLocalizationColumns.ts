...
export class WorkspaceLocalizationColumns1772499556166 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "workspaces" ADD "country" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "workspaces" ADD "state" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "workspaces" ADD "city" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "workspaces" ADD "taxId" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "workspaces" ADD "taxType" character varying`,
    );
  }
